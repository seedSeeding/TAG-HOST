    ...
    useEffect(() => {
        if (Array.isArray(patterns) && patterns.length > 0) {
            const filteredPattern = patterns.filter((pattern) => {
                return value && String(pattern).includes(value);
            });
    ...
    return (
        <div className="relative">
            ...
            {filtered.length > 0 && !selected && (
                <div className="mt-2 z-10 absolute w-full max-h-[150px] max-w-[140px] shadow-lg bg-[#373839] text-white">
                    <ul className="mt-2 rounded-lg p-2 w-full max-h-[150px] overflow-y-auto">
                        {filtered.map((pattern, index) => (
                            <li
                                key={index}
                                className="py-1 px-2 hover:bg-[#ECB22E] hover:text-white cursor-pointer text-[.9rem] w-full"
                                onClick={() => selectID(pattern)}
                            >
                                {pattern}
                            </li>
                        ))}
