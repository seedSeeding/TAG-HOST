...
class PatternDataController extends Controller
{
    ...
    public function getIssueAnalysis(Request $request)
    {
        try {
            ...
            foreach ($patterns as $pattern) {
                ...
                foreach ($pattern->{$category} as $item) {
                    ...
                    if ($item->approval_state === 'revision' && $item->size_id === $size_id) {
                        ...
                        $existingReasonIndex = array_search($reasonString, array_column($datas[$pattern->brand], 'reasons'));

                        if ($existingReasonIndex !== false) {
                    ...
                    } else if ($item->approval_state === 'revision' &&  $size_id === 5) {
            ...
            // Calculate the total frequency for each brand after gathering all reasons
            foreach ($datas as $brand => &$entries) {
                $totalFrequency = array_reduce($entries, function ($carry, $entry) {
                    return $carry + $entry['frequencies']['frequency'];
                }, 0);

                // Calculate the percentage for each entry based on its frequency
                foreach ($entries as &$entry) {
                    $entry['frequencies']['percentage'] = $totalFrequency > 0
                        ? number_format(($entry['frequencies']['frequency'] / $totalFrequency) * 100, 2)
                        : 0;
                }
            }
